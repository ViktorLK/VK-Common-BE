...
internal sealed class DefaultProfileStage : IVKPsychePipelineStage
{
    ...
    public async Task<VKResult> ExecuteAsync(VKPsycheContext context, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var profileResult = await _profileStore.GetProfileAsync(profileId.Value, cancellationToken).ConfigureAwait(false);
        ...
        {
            stopwatch.Stop();
            context.ResponseBuilder.ProfilingMetrics["ProfileStage"] = stopwatch.Elapsed.TotalMilliseconds;
        }
    }

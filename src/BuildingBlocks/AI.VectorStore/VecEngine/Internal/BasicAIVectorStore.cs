...
/// <summary>
/// Basic in-memory implementation of <see cref="IVKAIVectorStore"/>.
/// Following AP.03 Naming Taxonomy.
/// </summary>
internal sealed class BasicAIVectorStore : IVKAIVectorStore
{
    ...
    public IVKAIVectorCollection<T> Collection<T>(string name) where T : class
    {
        ...
        return new BasicAIVectorCollection<T>(name, this);
    }
    ...
    internal VKResult<VKAIVectorRecord<T>?> GetByIdGeneric<T>(string collectionName, string id) where T : class
    {
            ...
            !collection.TryGetValue(id, out var entry))
        {
            ...
            return VKResult.Success<VKAIVectorRecord<T>?>(null);
        }
        ...
        return VKResult.Success<VKAIVectorRecord<T>?>(new VKAIVectorRecord<T>(id, document, 1.0f));
    }

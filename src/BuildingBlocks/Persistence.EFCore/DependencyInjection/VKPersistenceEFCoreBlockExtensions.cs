...
public static class VKPersistenceEFCoreBlockExtensions
{
    ...
    /// <summary>
    /// Adds and configures the Persistence EF Core block using <see cref="IConfiguration"/> and a functional transform.
    /// </summary>
    public static IVKPersistenceEFCoreBuilder AddPersistenceEFCoreBlock<TContext>(
        this IServiceCollection services,
        IConfiguration configuration,
        ...
        where TContext : DbContext
    {
        return PersistenceEFCoreBlockRegistration.Register<TContext>(services, configuration, transform, dbContextOptions);
    }

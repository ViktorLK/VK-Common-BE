...
/// </summary>
internal sealed class AISemanticKernelAgent : AISemanticKernelProviderBase, IVKAgent
{
    private readonly VKAgentsOptions _options;

    internal ChatCompletionAgent InnerAgent { get; }

    public AISemanticKernelAgent(
        ...
        : base(kernel, modelName)
    {
        ...
        if (Tools.Count > 0)
        {
            var functions = Tools.Select(AISemanticKernelAgentToolAdapter.ToKernelFunction).ToArray();

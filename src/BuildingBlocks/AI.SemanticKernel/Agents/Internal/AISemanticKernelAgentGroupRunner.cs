...
/// </summary>
internal sealed class AISemanticKernelAgentGroupRunner : IVKAgentGroup
{
    private readonly Microsoft.SemanticKernel.Kernel _kernel;

    public AISemanticKernelAgentGroupRunner(Microsoft.SemanticKernel.Kernel kernel)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // 1. Resolve SK Agents from IVKAgent (ensure they are AISemanticKernelAgent instances)
            var skAgents = new List<ChatCompletionAgent>();
            foreach (var agent in agents)
            {
                if (agent is AISemanticKernelAgent AISemanticKernelAgent)
                {
                    skAgents.Add(AISemanticKernelAgent.InnerAgent);
                }
                else
                {

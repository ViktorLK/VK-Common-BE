...
/// </summary>
internal sealed class AISemanticKernelTextEngine : AISemanticKernelEngineBase<VKTextOptions>, IVKTextEngine
{
    private readonly ITextGenerationService? _textGeneration;
    private readonly IChatCompletionService? _chatCompletion;

    public AISemanticKernelTextEngine(
        Microsoft.SemanticKernel.Kernel kernel,
        IOptions<VKAIOptions> globalOptions,
        IOptions<VKTextOptions> textOptions,
        ILogger<AISemanticKernelTextEngine> logger,
    ...
    {
        var textArgs = args as VKTextArgs;
        ...
        if (textArgs is not null)
        {
            ...
            settings.ExtensionData["temperature"] = textArgs.Temperature ?? FeatureOptions.Temperature ?? 0.7f;
            settings.ExtensionData["max_tokens"] = textArgs.MaxTokens ?? FeatureOptions.MaxTokens ?? 512;
            if (textArgs.TopP.HasValue)
                settings.ExtensionData["top_p"] = textArgs.TopP.Value;
        }
    ...
    {
        AISemanticKernelMetrics.RecordChatDuration(durationSeconds, modelId); // Reuse chat duration for now or add specific text metrics

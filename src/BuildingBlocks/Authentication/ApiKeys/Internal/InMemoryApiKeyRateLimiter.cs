    ...
    TimeProvider timeProvider) : IVKApiKeyRateLimiter, IInMemoryCacheCleanup, IAsyncDisposable
{
    ...
    /// <inheritdoc />
    public Type AssociatedServiceType => typeof(IVKApiKeyRateLimiter);
    ...
    public ValueTask<bool> IsAllowedAsync(Guid keyId, int limit, int windowSeconds, CancellationToken ct = default)
    {
        ...
        RateLimitState state = _cache.GetOrAdd(keyId, _ => new RateLimitState());
        ...
        lock (state)
        {
            ...
            while (state.Timestamps.TryPeek(out long timestamp) && timestamp < windowStart)
            {

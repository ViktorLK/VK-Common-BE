...
{
    private static readonly ConcurrentDictionary<Type, Func<IEnumerable<Error>, object>> FailureFactories = new();

    private static readonly MethodInfo GenericResultFailureMethodInfo = typeof(Result)
    ...
    {
        var factory = FailureFactories.GetOrAdd(resultType, t =>
        {
            var genericMethod = GenericResultFailureMethodInfo.MakeGenericMethod(t);

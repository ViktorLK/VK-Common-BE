...
public sealed class VKPersistAggregateRepositoryGenerator : IIncrementalGenerator
{
    ...
    private static void EmitAggregateRepository(SourceProductionContext ctx, AggregateRepositoryInfo info)
    {
        ...
        sb.AppendLine("            var saveResult = await _unitOfWork.SaveChangesAsync(ct).ConfigureAwait(false);");
        sb.AppendLine("            if (saveResult.IsFailure)");
        sb.AppendLine("            {");
        sb.AppendLine("                activity?.SetStatus(ActivityStatusCode.Error, saveResult.FirstError.Description);");
        sb.AppendLine("                return VKResult.Failure(saveResult.FirstError);");
        sb.AppendLine("            }");
        sb.AppendLine();
        sb.AppendLine("            activity?.SetStatus(ActivityStatusCode.Ok);");

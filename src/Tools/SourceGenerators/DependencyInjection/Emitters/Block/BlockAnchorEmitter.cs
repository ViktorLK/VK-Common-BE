...
internal static class BlockAnchorEmitter
{
    public static void Emit(SourceProductionContext ctx, BlockTargetInfo target)
    {
        ...
        sb.AppendLine("        {");
        sb.AppendLine($"            throw new global::System.InvalidOperationException($\"Recursive block registration detected for {target.BlockName}Block! Do not call Register(builder) inside RegisterBlockCustom(builder).\");");
        sb.AppendLine("        }");

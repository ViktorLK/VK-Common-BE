...
public sealed class InMemoryJwtRefreshTokenValidatorTests
{
    ...
    public InMemoryJwtRefreshTokenValidatorTests()
    {
        ...
        _optionsMock.Setup(x => x.CurrentValue).Returns(new JwtOptions { RefreshTokenLifetimeDays = 7 });
    ...
    {
        // Arrange
        _optionsMock.Setup(x => x.CurrentValue).Returns(new JwtOptions { RefreshTokenLifetimeDays = 1 });
    ...
    {
        // Assert
        _validator.AssociatedServiceType.Should().Be(typeof(IJwtRefreshTokenValidator));
    }
}

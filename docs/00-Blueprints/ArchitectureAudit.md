...
- **BB.03 実行順序**: DI 登録の 8 ステップが正しい順序で実装されているか（Check-Self → Options → Mark-Self → Validator → Diagnostics → Toggle → Services）
...
- **Visibility 整合性**: Level 1 (public) / Level 2+ (internal) の境界が AP.03 に従っているか
- **Core 拡張と基盤抽象の徹底活用 (CS.01, 06, 12, 13)**: 車輪の再発明やシステムネイティブ API の直呼びを避け、Core が提供する標準抽象を最大限活用しているかを厳密に審査する。

...
[ADR-003](/docs/02-ArchitectureDecisionRecords/EFCore/adr-003-cursor-pagination.md) はカーソルページネーションの**アルゴリズム**（WHERE 句の動的生成・双方向スクロール）を決定した。その Future Considerations §1 において、以下の問題が予告されていた：
...
- 📄 [ADR-003: Cursor-Based Pagination](/docs/02-ArchitectureDecisionRecords/EFCore/adr-003-cursor-pagination.md) — 本 ADR の前提となるカーソルページネーション設計
- 📄 [ADR-002: Static Generic Caching](/docs/02-ArchitectureDecisionRecords/EFCore/adr-002-static-generic-caching.md) — `EfCoreExpressionCache` によるカーソルセレクタのコンパイルキャッシュ

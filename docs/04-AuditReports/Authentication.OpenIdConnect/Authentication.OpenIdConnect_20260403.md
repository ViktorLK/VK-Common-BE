...
public static IVKBlockBuilder<AuthenticationBlock> AddVKOidcBlock(
...
_注記_: `static class` は C# では暗黙的に `sealed` であるため、この指摘は実質的な影響なし。ただし、他のクラス（`OidcConstants`, `OidcDiagnosticsConstants`, `OidcDiagnostics`）も同様に `static` であるため問題なし。
...
| #    | 対象                                         | 問題                                            | 推奨アクション                                                                              |
| ---- | -------------------------------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------- |
| C-01 | `OidcAuthenticationBuilderExtensions.cs` L50 | `BuildServiceProvider()` アンチパターン         | `IConfiguration` を `AddVKOidcBlock` の引数として受け取るか、`PostConfigure` パターンに移行 |

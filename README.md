# VK-Common-BE

![.NET](https://img.shields.io/badge/.NET-10.0-512BD4)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
![Build Status](https://img.shields.io/badge/Build-Passing-brightgreen)

このリポジトリは、**モダンなバックエンドアーキテクチャの知識体系を構築・学習すること**を最大の目的としています。
単なるライブラリ集ではなく、DDD (ドメイン駆動設計) や Clean Architecture の原則に基づいた「実用的なビルディングブロック」の実装パターンを探求・蓄積しています。

---

## 🤖 AIエージェント開発エコシステム (AI-Agent Collaboration)

本プロジェクトは、AIエージェント（LLM）と人間が高度に協調して開発を行うための **AI Governance Protocol** を採用しています。特に [**VK.Blocks Checklist**](/.agents/rules/vk-blocks-checklist.md) は、プロジェクトの品質を支える最も重要な「憲法」です。

- **三層アーキテクチャ**: 絶対的な制約 (Rules)、思考プロセス (Workflows)、機械的処理 (MCP Tools) を分離。
- **階層制ルール (Tiered Strategy)**: L1 (全ルール意識) / L2 (即時禁令) / L3 (動的ロード) による厳格なコード品質管理。
- **Industrial DNA**: AIが `Result<T>`、非同期伝搬、`sealed` 等の工業基準を自動的に遵守・監査します。
- **Differential Loading (差分ロード)**: モジュールコンテキスト (`vk_get_module_context`) を優先的に Bootstrap し、不足する L3 ルールのみを最小限に「トップアップ（追記ロード）」する、エントロピー最小化プロトコルを採用。

詳細は [`.agents/README.md`](/.agents/README.md) を参照してください。

## 📦 現在の主要実装

### 🧱 [VK.Blocks.Core](/src/BuildingBlocks/Core/README.md)

すべての BuildingBlock モジュールの**基盤レイヤー（Foundation Layer）**です。
DI 登録パイプライン（マーカーパターン・再帰的依存解決・冪等二重登録）、Result Pattern（Railway-Oriented Programming）、VKGuard（Fluent ガード節）、ドメインプリミティブ（Entity / ValueObject / AggregateRoot）、OpenTelemetry 診断基盤など、フレームワーク全体のクロスカッティング・コンサーンを一元的に提供します。
ゼロリフレクション設計と `stackalloc` / `HashCode` 構造体によるアロケーションフリー実装により、エンタープライズ品質のパフォーマンスを実現しています。

👉 **[詳細な設計思想とドキュメントはこちら](/src/BuildingBlocks/Core/README.md)**

---

### 🔐 [VK.Blocks.Authentication](/src/BuildingBlocks/Authentication/README.md)

構成駆動型 (Configuration-Driven) の**マルチ戦略認証基盤**です。
JWT、API Key、OAuth の各認証を同一のパイプラインで統合管理し、型安全なセマンティック認証属性 (`[JwtAuthorize]` 等) を提供します。
高パフォーマンスなハッシュ生成や自己適応型のキャッシュクリーンアップなど、エンタープライズ品質のセキュリティ機能を備えています。

👉 **[詳細な設計思想とドキュメントはこちら](/src/BuildingBlocks/Authentication/README.md)**

---

### 🌐 [VK.Blocks.Authentication.OpenIdConnect](/src/BuildingBlocks/Authentication.OpenIdConnect/README.md)

`VK.Blocks.Authentication` の**OIDC フェデレーション拡張モジュール**です。
Azure AD B2C・Google・Auth0 等の外部 Identity Provider を `appsettings.json` の設定のみで動的に登録し、Keyed DI によるプロバイダー別クレームマッピングと Fail-Fast 起動時検証を提供します。
コアモジュールへの逆依存を完全に排除した、独立デプロイ可能な Vertical Slice 設計です。

👉 **[詳細な設計思想とドキュメントはこちら](/src/BuildingBlocks/Authentication.OpenIdConnect/README.md)**

---

### 🛡️ [VK.Blocks.Authorization](/src/BuildingBlocks/Authorization/README.md)

ASP.NET Core の認可パイプラインを拡張した、**多次元認可基盤**です。
Vertical Slice Architecture により、パーミッション・ロール・テナント分離・職位ランク・勤務時間帯制限・内部ネットワーク制御・動的ポリシーの7つの認可機能を独立した Feature として実装しています。
`IAuthorizationRequirementData` による型安全な宣言的属性 (`[AuthorizePermission]`, `[AuthorizeRoles]`, `[DynamicAuthorize]`) と `Result<T>` パターンを統合し、すべての評価結果を OpenTelemetry メトリクスで自動計測します。

👉 **[詳細な設計思想とドキュメントはこちら](/src/BuildingBlocks/Authorization/README.md)**

---

### 🤖 [VK.Blocks.AI](/src/BuildingBlocks/AI/README.md)

OpenAI、Azure OpenAI、Google Gemini、Anthropic Claude、Ollama 等の LLM プロバイダーに依存しない**プロバイダー非依存 AI 抽象ライブラリ**です。
「抽象のみ・実装ゼロ」の設計思想に基づき、アプリケーション層から AI インフラの実装詳細を完全に分離します。
Chat (Streaming 対応)、Multi-Modal Message (Text/Image/Audio/File)、Embedding、Moderation、Tokenics (使用量/コスト追跡)、そして AP.05 に基づく階層的構成パターンを標準提供します。

👉 **[詳細な設計思想とドキュメントはこちら](/src/BuildingBlocks/AI/README.md)**

---

### 🗄️ [VK.Blocks.Persistence.EFCore](/src/BuildingBlocks/Persistence/EFCore/README.md)

Entity Framework Core をベースとした、高機能かつ堅牢な永続化層モジュールです。
バルク操作と監査ログの自動整合 (Hybrid Auditing) や、スレッドセーフなメタデータキャッシュなど、パフォーマンスと保守性を両立させる工夫を凝らしています。

👉 **[詳細な設計思想とドキュメントはこちら](/src/BuildingBlocks/Persistence/EFCore/README.md)**

---

## 🛠️ 開発者ツール (Developer Tooling)

### 🧱 [VK.Tools.McpServer](/src/Tools/McpServer/README.md)

VK.Blocks エコシステムにおける **AI 連携の「メカニクス（機械的処理）」** を担当する MCP サーバーです。
[ADR-001](/docs/02-ArchitectureDecisionRecords/DeveloperEcosystem/adr-001-establish-tri-layered-ai-agent-collaboration-architecture.md) に基づく三層アーキテクチャのうち、AI エージェントがプロジェクトの構造、ルール、および管理タスクに対して確定的な操作を行えるようにするための「ガードレール」として機能します。

- **BuildingBlock 管理**: モジュールの依存関係解析とボイラープレート生成支援。
*   **ADR & ルールガバナンス**: アーキテクチャ決定記録 (ADR) のドラフト作成とテンプレート管理。
*   **バックログ同期**: `docs/Backlogs/` へのタスク項目の自動追加と進捗同期。
*   **高度な監査**: EF Core 破壊的変更の検出、テストカバレッジ分析、AI 向けコードベースエクスポート。

👉 **[詳細な機能とセットアップはこちら](/src/Tools/McpServer/README.md)**
